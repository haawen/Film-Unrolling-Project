#!/bin/bash
#SBATCH --job-name=fixleft
#SBATCH --partition=daily
#SBATCH --time=03:00:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --output=/data/user/li_k1/M_thesis/unwrapping/eval/results/fixleft_%j.out
...
OUT=unwrapping/eval/results/stabilization/renders/fixleft
mkdir -p "$OUT"
...
python -m unwrapping.eval.border_render "$STRIP" $COMMON \
    --margin 0.0 --video film_fixleft_m00.mp4

echo "=== margin 0.02: thin sliver so the border stays visible as proof ==="
python -m unwrapping.eval.border_render "$STRIP" $COMMON \
    --margin 0.02 --video film_fixleft_m02.mp4
