...
V={"ORIGINAL":read(IN),"UDVD s15 (softer)":read(RD+"/udvd_zs_s15.mp4"),"UDVD s25":read(RD+"/udvd_zs_s25.mp4")}
...
def row(imgs,names,zoom=None):
    ...
    return r
rows=[row([V[k][150] for k in V],list(V.keys())),
      row([V[k][150] for k in V],[f"{k} [zoom]" for k in V],zoom=(int(H*0.15),int(W*0.55),220))]
...
Image.fromarray(full).save(RD+"/compare_newscan_s15_vs_s25.png"); print("saved",full.shape)
