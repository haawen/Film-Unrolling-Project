...
def walk_anchor(seg2, cx, cy, r0, r1, film_min_thick, snap_max, step, coast_max,
                ...
                rescue=True, rescue_min_sep=6.0, max_infill=3,
                walk_class="emulsion"):
    ...
    # WHAT THE WALK FOLLOWS.  Normally the class-2 emulsion, which is where the
    # picture is and is what every delivered render used.  In the PERFORATION
    # bands the emulsion class comes out fragmentary while the film base is
    # still found cleanly (with the holes punched through it), so there `film`
    # is the followable structure.  This only swaps the target the walk snaps
    # to and re-centres on; seeding still uses the film band either way.
    target = emul if walk_class == "emulsion" else film
    ...
    def walk_from(p0, az):
        ...
        fwd = walk(target, p0, tan, cx, cy, step, snap_max, coast_max, fwd_target,
                   jump_max=jump_max, snap_accept=snap_accept, max_dr=max_dr)
        bwd = walk(target, p0, -tan, cx, cy, step, snap_max, coast_max, bwd_target,
                   jump_max=jump_max, snap_accept=snap_accept, max_dr=max_dr)
        p = smooth_path(np.vstack([bwd[::-1], fwd[1:]]), smooth_px)
        if recenter:
            p = recenter_on_emulsion(p, target, recenter_search, recenter_sigma)
        return p
    ...
    paths = [walk_from(np.array([cx + rs * math.cos(seed_az),
             ...
             for rs in seeds]
    if rescue:
        paths = rescue_bare_bands(paths, target, cx, cy, seam, eps,
...
def arc_equalize(X, Y, cx_a, cy_a, phi_grid, px_per_col=1.0):
    """Re-grid a winding's columns from uniform AZIMUTH to uniform ARC LENGTH.

    THE v13 FIX. resample_phi puts the columns on a uniform phi grid, so each
    column covers ds/dphi = hypot(r, dr/dphi) px of film — and r swings by the
    winding's eccentricity (~80px ptp, measured on v12), so the film length per
    column swings with it. Measured on the v12 geometry: the local scale runs
    0.914..1.060 (+-5%), COHERENTLY across all 36 windings, once per turn; the
    along-film position error peaks at ~90px = 0.1 film cell per turn. On the
    delivered v12 strip the local cell pitch really does oscillate 886..913px
    with a one-winding period — i.e. the picture breathes and the framing creeps
    back and forth every ~6.7 frames. That is a PARAMETERIZATION error, not a
    fit error (the fitted curves are smooth to 0.23px rms).

    Fix: choose the columns so each one spans the same ARC LENGTH of film, using
    the Z-MEDIAN radius profile. Because the resulting phi-per-column map is
    computed once and shared by every z, the cross-z alignment that resample_phi
    exists to provide is preserved EXACTLY — this only changes where the columns
    sit along the turn, not which film position a column means.

    Returns (phi_new, X2, Y2)."""
    if X.shape[1] < 4:
        return phi_grid, X, Y
    # Arc length of the z-MEDIAN CURVE, measured as the polyline it actually is.
    # NOT the analytic hypot(r, dr/dphi)*dphi: resample_phi CLAMPS a partial
    # walk's out-of-span columns to its endpoint, and the analytic form reads
    # that repeated point as r*dphi of film that does not exist — measured at
    # +2.7% of the whole roll, all of it from the two film-end fragment windings
    # (w0 51% clamped, w1 93%), which inflated the strip to 240 cells vs GT ~233.
    # A polyline has zero length across a repeated point, so it is immune. It is
    # also center-free, like the walk itself.
    #
    # The curve is lightly smoothed FIRST: columns are ~1px apart, so raw
    # per-column steps are dominated by sub-pixel jitter, and |jitter| only ever
    # ADDS length (a biased over-estimate). Smoothing at sigma 15 columns is far
    # below the once-per-turn modulation we are correcting (~6000 columns) and
    # its chord-shortening on a ~1000px-radius curve is ~4e-5 — negligible.
    from scipy.ndimage import gaussian_filter1d
    xm = gaussian_filter1d(np.median(X, axis=0).astype(float), 15, mode="nearest")
    ym = gaussian_filter1d(np.median(Y, axis=0).astype(float), 15, mode="nearest")
    step = np.hypot(np.diff(xm), np.diff(ym))
    s = np.concatenate([[0.0], np.cumsum(step)])
    if s[-1] <= 0:
        return phi_grid, X, Y
    K2 = max(2, int(round(s[-1] / max(1e-6, px_per_col))))
    phi_new = np.interp(np.linspace(0.0, s[-1], K2), s, phi_grid)
    n = X.shape[0]
    X2 = np.empty((n, K2), X.dtype); Y2 = np.empty((n, K2), Y.dtype)
    for a in range(n):
        X2[a] = np.interp(phi_new, phi_grid, X[a])
        Y2[a] = np.interp(phi_new, phi_grid, Y[a])
    return phi_new, X2, Y2
...
def z_heal_windings(kept, cx_a, cy_a, seam, eps_phi, thresh, window=61,
    ...
    if w < 5:
        return kept, 0
    healed, out = 0, []
    for wi, (r0, X, Y, phi) in enumerate(kept):
        ...
        if wi == 0 or wi == len(kept) - 1:
            out.append((r0, X, Y, phi))
            continue
        K = X.shape[1]
        # the column's ray angle — read from the winding's OWN phi grid, which
        # v13 no longer assumes is uniform (see arc_equalize).
        th = (seam + np.asarray(phi, float))[None, :]
...
def _z_map(z_anchor, A, mode="hold", span=500.0):
    """Map z -> winding path, with a choice of what happens OUTSIDE the anchors.

    Inside the anchor range this is the usual `interp1d`.  Outside it:

      hold    repeat the end anchor (the historic v13 behaviour, and what every
              delivered v9/v12/v13 render used).
      linear  continue the path's own z-trend, fitted by least squares over the
              anchors within `span` slices of the end.

    Why this matters: on the full-width new scan the walk anchors only cover the
    PICTURE band, so `hold` renders both perforation rows on picture-band
    geometry.  The holes then come out displaced by 4-44 px in a way that does
    NOT correlate with the picture band's own motion (r = +0.03), which is
    exactly what stops the sprockets working as a registration fiducial.
    `Scripts/diag_z_extrap_cv.py` cross-validated the two: at 350 slices out,
    linear gives 2.0 px against hold's 3.6 px.

    `span` is in SLICES, not in number of anchors.  That distinction is the trap
    here: the CV's "F=100" was 100 anchors on a run with one anchor per slice,
    i.e. 100 slices.  On this render the 160 anchors are 8 clusters spread over
    1400 slices, so "100 anchors" would be a 1400-slice window and "the nearest
    100 slices" would be a single 20-slice chunk -- far too short a baseline to
    fit a slope on.  The default 500 covers ~3 chunks at each end, which is
    longer than the 55-271 slices the perf bands actually need extrapolated, so
    the fit is interpolating its own trend rather than reaching.

    NOTE this changes ONLY the region outside the anchors; the picture band is
    bit-for-bit unaffected, so it cannot degrade the picture.
    """
    inner = interp1d(z_anchor, A, axis=0, bounds_error=False,
                     fill_value=(A[0], A[-1]))
    z_anchor = np.asarray(z_anchor, float)
    if mode == "hold" or len(z_anchor) < 4:
        return inner

    z0, z1 = z_anchor[0], z_anchor[-1]
    lo_m = z_anchor <= z0 + span
    hi_m = z_anchor >= z1 - span
    if lo_m.sum() < 3 or hi_m.sum() < 3:            # not enough to fit a slope
        return inner
    ml = z_anchor[lo_m].mean(); mh = z_anchor[hi_m].mean()
    pl = np.polyfit(z_anchor[lo_m] - ml, A[lo_m], 1)     # (2, K)
    ph = np.polyfit(z_anchor[hi_m] - mh, A[hi_m], 1)

    def _f(z):
        zs = float(z)
        if zs < z0:
            return pl[0] * (zs - ml) + pl[1]
        if zs > z1:
            return ph[0] * (zs - mh) + ph[1]
        return inner(zs)
    return _f


def _blend(wa, wb, w, eps_phi):
    """Midline between two windings (r, X(n,K), Y(n,K), phi(K)) at w in [0,1].
    ...
    v13: the two flanking windings no longer share a column grid (each is on its
    own arc-length grid), so blend on a common uniform-phi grid; the caller
    re-equalizes the result to arc length."""
    ra, Xa, Ya, pa = wa; rb, Xb, Yb, pb = wb
    K = (Xa.shape[1] + Xb.shape[1]) // 2
    phi = np.linspace(eps_phi, TWO_PI - eps_phi, K)
    n = Xa.shape[0]
    Xa2 = np.empty((n, K), np.float32); Ya2 = np.empty((n, K), np.float32)
    Xb2 = np.empty((n, K), np.float32); Yb2 = np.empty((n, K), np.float32)
    for a in range(n):
        Xa2[a] = np.interp(phi, pa, Xa[a]); Ya2[a] = np.interp(phi, pa, Ya[a])
        Xb2[a] = np.interp(phi, pb, Xb[a]); Yb2[a] = np.interp(phi, pb, Yb[a])
    return phi, ((1 - w) * Xa2 + w * Xb2), ((1 - w) * Ya2 + w * Yb2)
...
def main():
    ...
    ap.add_argument("--coast-max", type=int, default=40,
                    help="Steps the walk may coast through a gap with no target "
                         "under it. In ARC PX this is coast_max * step, so the "
                         "50 used on the new scan buys 125 px -- and a 16 mm "
                         "perforation is 188 px of film along its length, which "
                         "is why the walk gives up in the perf bands. ~120 "
                         "(300 px) clears a hole with margin.")
    ap.add_argument("--walk-class", choices=["emulsion", "film"],
                    default="emulsion",
                    help="What the walk follows and re-centres on. emulsion "
                         "(class 2) is the picture layer and is what every "
                         "delivered render used. film (class >= 1, base + "
                         "emulsion) is for the PERFORATION bands, where the "
                         "emulsion class is fragmentary but the base is found "
    ...
    ap.add_argument("--max-infill", type=int, default=3,
                    help="Max windings ONE seed gap may claim to hide. A wider gap "
                         "is treated as a void (bore / outside the film) and filled "
                         "with nothing. Unbounded infill turned a single spurious "
                         "bore detection on the new scan into 33 fake windings "
                         "(37 real -> 71 seeds, tracks at r=18..759 where the film "
                         "starts at r=776).")
    ap.add_argument("--radial-robust", action="store_true",
                    help="Take the seeding radial range from azimuthal COVERAGE "
                         "instead of min/max over labelled pixels, so bore artefacts "
                         "cannot set r0. Off by default = the behaviour that produced "
                         "the delivered v9/v12/v13 old-scan results.")
    ap.add_argument("--radial-cov", type=float, default=0.3,
                    help="--radial-robust: min fraction of azimuths carrying film "
                         "for a radius to count as inside the annulus")
    ap.add_argument("--radial-cov-outer", type=float, default=None,
                    help="Looser coverage threshold for the OUTER edge only "
                         "(default: same as --radial-cov). The outermost wrap is a "
                         "PARTIAL turn -- the film's outer end stops part way round "
                         "-- so it scores well under 0.3 and the annulus is cut "
                         "inside it, leaving a visible untraced arc (measured on the "
                         "low band: gate r 752-1720 against raw film out to 1728). "
                         "Lowering --radial-cov itself would break what it is for: "
                         "keeping bore artefacts from setting r0, where unbounded "
                         "infill once turned 37 real windings into 71. So relax the "
                         "outer edge alone, and only outward-contiguously and by at "
                         "most --radial-outer-max-px.")
    ap.add_argument("--radial-outer-max-px", type=float, default=40.0,
                    help="Cap on how far --radial-cov-outer may push the outer "
                         "radius past the strict gate. ~1.5 winding pitches on the "
                         "new scan (26px), so it can admit one partial wrap and "
                         "cannot run away into noise outside the roll.")
    ap.add_argument("--n-centers", type=int, default=30,
                    help="TOTAL find_spool_center calls over the whole stack, evenly "
                         "spread in z, interpolated to every anchor. NOT per chunk. "
                         "Previously 1 per chunk (25 gapped / 60 full-scroll) then "
                         "z-smoothed. 30 keeps most of that sampling (so per-call "
                         "noise still averages out along the track) at ~75min on the "
                         "new scan, where a call costs ~2.5min (~63s on the old "
                         "3063^2 slices). Samples are taken from chunks already in "
                         "memory, so they add no extra I/O.")
    ...
    ap.add_argument("--no-arc-columns", dest="arc_columns", action="store_false",
                    help="v13 (default ON): put the strip columns on uniform ARC "
                         "LENGTH instead of uniform AZIMUTH. resample_phi's uniform "
                         "phi grid makes each column cover hypot(r, dr/dphi) px of "
                         "film, which swings with the winding's eccentricity: "
                         "measured +-5%% within every turn on the v12 geometry, "
                         "coherent across all 36 windings, ~90px (0.1 cell) of "
                         "along-film position creep per turn — and the delivered "
                         "v12 strip's local cell pitch really does oscillate "
                         "886..913px with a one-winding period. That is the "
                         "residual video breathing/wobble, and it is a "
                         "PARAMETERIZATION error, not a fit error (the fitted "
                         "curves are smooth to 0.23px rms). Pass this flag to get "
                         "the legacy v12 uniform-phi columns for an A/B.")
    ap.add_argument("--z-extrap", choices=["hold", "linear"], default="hold",
                    help="What the render does OUTSIDE the walk anchors' z "
                         "range. hold = repeat the end anchor (the historic "
                         "behaviour; every delivered v9/v12/v13 render used it, "
                         "hence the default). linear = continue each path's own "
                         "z-trend, which is what the perf bands of the "
                         "full-width scan need. See _z_map().")
    ap.add_argument("--z-extrap-span", type=float, default=500.0,
                    help="Slices (NOT anchors) at each end used to fit the "
                         "linear z-trend. See the trap noted in _z_map().")
    ap.add_argument("--transverse-px", type=int, default=2,
                    help="Average this many samples across the emulsion's own "
                         "THICKNESS (taps spaced 1px along the path normal, "
                         "centred on the path). 1 = the single-line point sample "
                         "used through v12. The class-2 emulsion measures 4.0px "
                         "median (p10 3.2, p90 5.0) on real Mickey, so 3 stays "
                         "inside a median band and 5 exceeds even a p90 one. "
                         "DEFAULT 2 (taps at +-0.5px, ~2px support) from the GT "
                         "sweep tw1/2/3/5: quality saturates fast but blur cost "
                         "keeps accruing (FID 156/157/163/182), so 2 sits at the "
                         "knee -- it takes ~96%% of tw3's NIQE gain (4.84->4.33 "
                         "vs 4.31) and BRISQUE 19.8->16.6, while being the ONLY "
                         "width that also BEATS tw1 on grad_corr/PSNR/NMI instead "
                         "of trading them away. 3 is defensible if you weight "
                         "DISTS/LPIPS/MS-SSIM most (it wins those by ~0.002-0.005); "
                         "5 is over-smoothed. Set 1 for the strictly-faithful "
                         "point-sampled baseline.")
    ap.add_argument("--px-per-col", type=float, default=1.0,
                    help="With arc columns: film px per strip column (1.0 = native "
                         "CT resolution, and what v12's ~1.0015 px/col already "
                         "averaged). Same for every winding, so the scale is now "
                         "constant over the WHOLE roll, not just per winding.")
    ...
    if args.use_cached_anchors and os.path.exists(cache):
        ...
        import time
        # ── SPOOL CENTRES: `--n-centers` calls over the WHOLE stack (not per chunk),
        #    evenly spread in z, then interpolated to every slice. The centre drifts
        #    smoothly and slowly along the roll, so a few well-separated samples
        #    describe it; per-chunk sampling would cost ~2.5min x 3 x n_chunks (~9h
        #    on the 72-chunk new scan) and add per-chunk noise, since a single
        #    find_spool_center call scatters by tens of px (measured: a 20-slice
        #    chunk gave (1893,1843), (1879,1904), (1870,1902)). ──
        chunk_pairs = pairs[:args.max_chunks] if args.max_chunks else pairs
        n_ctr = max(1, min(args.n_centers, len(chunk_pairs)))
        pick = set(np.unique(np.linspace(0, len(chunk_pairs) - 1,
                                        n_ctr).astype(int)).tolist())
        # Sampled chunks are centred FROM THE CHUNK ALREADY IN MEMORY during the walk
        # loop -- a separate pre-pass would re-read ~3min of probabilities per sampled
        # chunk (~90min of pure I/O at 30 samples). Inside the loop each chunk walks
        # with the latest sample as a provisional centre (the walk is centre-FREE
        # after seeding: the centre only sets the seed ray's radial window, so tens of
        # px are harmless there); the per-anchor centres are then REPLACED by the
        # interpolation over all samples once they exist, before anything that is
        # centre-sensitive runs (median-radius tracking, resample_phi's common azimuth
        # grid, arc_equalize).
        cz, ccy, ccx = [], [], []
        t_c0 = time.time()
        ...
        for ci, (vp, pp, z0, z1) in enumerate(pairs):
            ...
            t_load = time.time() - t0
            t_ctr = 0.0
            if ci in pick:                     # centre this chunk (already loaded)
                t1 = time.time()
                s_mid, si_mid = slices[len(slices) // 2]
                cyi, cxi = find_spool_center(s_mid)
                cz.append(float(z0 + si_mid)); ccy.append(cyi); ccx.append(cxi)
                t_ctr = time.time() - t1
                print(f"    centre sample {len(cz)}/{n_ctr} at z{cz[-1]:.0f}: "
                      f"{cyi:.1f},{cxi:.1f} ({t_ctr:.0f}s)", flush=True)
            if not cz:                          # first chunk is always in `pick`
                raise RuntimeError("no centre sample yet -- pick must include chunk 0")
            cy_prov, cx_prov = ccy[-1], ccx[-1]
            t_walk = 0.0
            for si_i, (seg2, sidx) in enumerate(slices):
                cy, cx = cy_prov, cx_prov
                ys, xs = np.where(seg2 > 0); rf = np.hypot(ys - cy, xs - cx)
                if args.radial_robust:
                    # r0/r1 from AZIMUTHAL COVERAGE, not min/max over all labelled
                    # pixels: a handful of bore artefacts (or noise outside the roll)
                    # otherwise drags r0 to ~13px and the seeding then fills the
                    # empty core with fake windings. The film annulus is where a
                    # decent fraction of azimuths carry film.
                    nb = 200
                    edges = np.linspace(0, rf.max() + 1, nb + 1)
                    which = np.clip(np.digitize(rf, edges) - 1, 0, nb - 1)
                    th = np.arctan2(ys - cy, xs - cx)
                    abin = np.clip(((th + math.pi) / TWO_PI * 72).astype(int), 0, 71)
                    cov = np.zeros(nb)
                    seen = np.zeros((nb, 72), bool)
                    seen[which, abin] = True
                    cov = seen.mean(axis=1)
                    hit = np.nonzero(cov > args.radial_cov)[0]
                    if hit.size >= 2:
                        r0_c = float(edges[hit[0]]); r1_c = float(edges[hit[-1] + 1])
                        # Outer edge only: walk outward while consecutive bins stay
                        # above the looser threshold, capped in px. The inner edge
                        # keeps the strict threshold -- that is the bore guard.
                        co = args.radial_cov_outer
                        r1_strict = r1_c
                        if co is not None and co < args.radial_cov:
                            j = int(hit[-1])
                            while j + 1 < nb and cov[j + 1] > co:
                                j += 1
                            r1_ext = float(edges[j + 1])
                            if r1_ext - r1_strict <= args.radial_outer_max_px:
                                r1_c = r1_ext
                        if si_i == 0:
                            extra = ""
                            if r1_c > r1_strict:
                                extra = (f" [outer edge extended {r1_c - r1_strict:.0f}px "
                                         f"by coverage>{co}]")
                            print(f"    radial range: coverage>{args.radial_cov} -> "
                                  f"r {r0_c:.0f}-{r1_c:.0f} (raw min/max "
                                  f"{rf.min():.0f}-{rf.max():.0f}){extra}", flush=True)
                        rf = rf[(rf >= r0_c) & (rf <= r1_c)]
                tw = time.time()
        ...
        # ── final centres: interpolate the samples over z and REPLACE the provisional
        #    per-anchor values (see the note above). Done before tracking /
        #    resample_phi / arc_equalize, all of which are centre-sensitive. ──
        cz = np.array(cz, float); ccy = np.array(ccy); ccx = np.array(ccx)
        if len(cz) >= 2:
            cy_a = list(np.interp(z_anchor, cz, ccy))
            cx_a = list(np.interp(z_anchor, cz, ccx))
        else:
            cy_a = [float(ccy[0])] * len(z_anchor)
            cx_a = [float(ccx[0])] * len(z_anchor)
        msg = (f"  centres: {len(cz)} samples over z{cz[0]:.0f}-{cz[-1]:.0f} in "
               f"{time.time() - t_c0:.0f}s, interpolated to {len(z_anchor)} anchors; "
               f"end-to-end drift "
               f"{np.hypot(ccy[-1] - ccy[0], ccx[-1] - ccx[0]):.1f}px")
        if len(cz) >= 3:
            A = np.stack([cz, np.ones_like(cz)], 1)
            ry = ccy - A @ np.linalg.lstsq(A, ccy, rcond=None)[0]
            rx = ccx - A @ np.linalg.lstsq(A, ccx, rcond=None)[0]
            msg += (f"; max residual from a straight line in z "
                    f"{max(np.abs(ry).max(), np.abs(rx).max()):.1f}px (large = an "
                    f"outlier sample; a single find_spool_center call scatters by "
                    f"tens of px on the new scan)")
    ...
    for k in range(nw):
        ...
        if len(present) < max(2, args.min_coverage):   # drop spurious low-cov windings
            dropped.append((float(ref_r[k]), len(present)))
            continue
        ...
        kept.append((float(ref_r[k]), X, Y, phi_grid))
    kept.sort(key=lambda t: t[0])
    print(f"  {len(kept)} windings kept (coverage >= {args.min_coverage})")
    # Report the dropped tracks NEXT TO the radius gaps they leave behind. A gap
    # wider than ~1.5x the median is what triggers gap-infill below, and infill
    # fabricates a winding by blending its neighbours -- so if a real, merely
    # under-covered track sits inside that gap, the honest fix is to keep it
    # (lower --min-coverage) rather than to synthesise one on top of it.
    if dropped:
        dropped.sort()
        radii = [t[0] for t in kept]
        med_g = float(np.median(np.diff(radii))) if len(radii) > 2 else 0.0
        print(f"  {len(dropped)} tracks dropped below coverage {args.min_coverage}"
              f" (median kept spacing {med_g:.1f}px):")
        for r, c in dropped:
            inside = ""
            for i in range(1, len(radii)):
                if radii[i - 1] < r < radii[i] and med_g > 0 and \
                        (radii[i] - radii[i - 1]) > 1.5 * med_g:
                    inside = (f"   <-- INSIDE a {radii[i]-radii[i-1]:.0f}px gap "
                              f"(r {radii[i-1]:.0f}-{radii[i]:.0f}) that gap-infill "
                              f"will otherwise FABRICATE")
                    break
            print(f"    r {r:7.1f}  coverage {c:5d}/{n}{inside}")
    ...
    if args.gap_infill and len(kept) >= 3:
        ...
        for i in range(1, len(kept)):
            ...
            for j in range(1, m + 1):
                w = j / (m + 1)
                pb, Xb, Yb = _blend(kept[i - 1], kept[i], w, eps_phi)
                filled.append((kept[i - 1][0] + g * w, Xb, Yb, pb)); n_ins += 1
    ...
    if args.z_smooth_anchors > 0 and n >= 3:
        ...
        kept = [(r, gaussian_filter1d(X, s, axis=0, mode="nearest"),
                 gaussian_filter1d(Y, s, axis=0, mode="nearest"), p)
                for (r, X, Y, p) in kept]
        print(f"  z-smoothed winding paths across anchors (sigma {s} anchors)")

    # ── v13: ARC-LENGTH columns (the fix — see arc_equalize). Runs LAST, on the
    #    final healed/smoothed geometry, so the profile it equalizes is the one
    #    actually rendered. ──
    if args.arc_columns:
        eq, before = [], sum(t[1].shape[1] for t in kept)
        stretch = []
        for (r, X, Y, p) in kept:
            xm = np.median(X, axis=0).astype(float)
            ym = np.median(Y, axis=0).astype(float)
            ds = np.hypot(np.diff(xm), np.diff(ym))     # px of film per column
            core = ds[len(ds) // 20: -len(ds) // 20] if len(ds) > 40 else ds
            core = core[core > 1e-6]                    # ignore clamped (zero-length)
            if core.size:
                stretch.append(float(np.percentile(core, 99) /
                                     max(np.percentile(core, 1), 1e-6)))
            p2, X2, Y2 = arc_equalize(X, Y, cx_a, cy_a, p, args.px_per_col)
            eq.append((r, X2, Y2, p2))
        kept = eq
        after = sum(t[1].shape[1] for t in kept)
        print(f"  arc-columns: uniform-phi -> uniform arc length "
              f"({args.px_per_col} px/col); cols {before} -> {after}. "
              f"Removed a within-turn stretch of med "
              f"{100 * (np.median(stretch) - 1) / 2:+.1f}% / max "
              f"{100 * (max(stretch) - 1) / 2:+.1f}% (peak-to-mean).")
    ...
    if args.inspect_anchors:
        ...
        mp[:] = [[np.column_stack([X[a], Y[a]]) for (_, X, Y, _p) in kept] for a in range(n)]
        ...
        for a in ins_idx:
            ...
            for kk, (_, X, Y, _p) in enumerate(kept):
    ...
    win = [(_z_map(z_anchor, X, args.z_extrap, args.z_extrap_span),
            _z_map(z_anchor, Y, args.z_extrap, args.z_extrap_span),
            X.shape[1]) for (_, X, Y, _p) in kept]
    if args.z_extrap != "hold":
        print(f"  z-extrapolation OUTSIDE the anchor range "
              f"[{z_anchor[0]:.0f}, {z_anchor[-1]:.0f}]: {args.z_extrap}, "
              f"fitted over the nearest {args.z_extrap_span:.0f} slices", flush=True)
    z_list = sorted(z_to_path)[:: args.z_step]
    strip = np.empty((len(z_list), total_cols), dtype=np.float32)
    # TRANSVERSE AVERAGING across the emulsion's own thickness. The render
    # otherwise reads a single infinitesimally-thin line through the middle of a
    # band measured at 4.0px median (p10 3.2, p90 5.0) on real Mickey -- note
    # that is the CLASS-2 EMULSION; the ~18px figure is the class>=1 FILM layer
    # (base + emulsion). Averaging n taps along the path NORMAL integrates
    # through the emulsion the way a densitometer reads film: ~sqrt(n) better
    # SNR, and far less sensitive to sub-pixel path error or to exactly where
    # the segmentation puts the band centre. Risk: where the emulsion is tilted
    # w.r.t. the path normal, or where taps spill past a 3.2px band into the
    # base, this smears along the film -- hence the width sweep.
    tw = max(1, int(args.transverse_px))
    offs = np.linspace(-(tw - 1) / 2.0, (tw - 1) / 2.0, tw)
    if tw > 1:
        print(f"  transverse averaging: {tw} taps at offsets "
              f"{np.round(offs, 2).tolist()} px along the path normal")
    for j, z in enumerate(z_list):
        ...
        for fX, fY, K in win:
            xs = fX(z); ys = fY(z)
            if tw == 1:
                strip[j, c:c + K] = bilinear_sample(img, xs, ys)
            else:
                dx = np.gradient(xs); dy = np.gradient(ys)
                L = np.hypot(dx, dy) + 1e-9
                nx, ny = -dy / L, dx / L           # unit normal, per column
                acc = np.zeros(K, np.float32)
                for t in offs:
                    acc += bilinear_sample(img, xs + t * nx, ys + t * ny)
                strip[j, c:c + K] = acc / tw
            c += K
    ...
    if args.seam_bridge and eps_phi > 0 and len(win) >= 2:
        ...
        for i, K in enumerate(Ks):
            ...
            if i < len(Ks) - 1:                       # not after the outermost
                if args.arc_columns:
                    # arc columns are px_per_col of FILM each, so the missing
                    # wedge is simply its arc length r_seam*2*eps / px_per_col.
                    # (The uniform-phi formula below would over/under-fill by the
                    # same +-5% the arc re-grid just removed.)
                    _, Xk, Yk, _p = kept[i]
                    r_seam = float(np.median(np.hypot(
                        Xk[:, -1] - np.asarray(cx_a, float),
                        Yk[:, -1] - np.asarray(cy_a, float))))
                    M = int(round(r_seam * 2 * eps_phi / max(1e-6, args.px_per_col)))
                else:
                    # missing arc = 2*eps rad; match cols/rad = K/(2pi-2eps)
